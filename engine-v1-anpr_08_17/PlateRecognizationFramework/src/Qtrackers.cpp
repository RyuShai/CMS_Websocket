...
size_t QTrack::NextTrackID = 0;
// ---------------------------------------------------------------------------
// Track constructor.
// The track begins from initial point (pt)
// ---------------------------------------------------------------------------
QTrack::QTrack(Point2f pt, float dt, float Accel_noise_mag)
{
...
// ---------------------------------------------------------------------------
QTrackers::QTrackers(float _dt, float _Accel_noise_mag, double _dist_thres, int _maximum_allowed_skipped_frames,int _max_trace_length)
{
dt=_dt;
...
//
// ---------------------------------------------------------------------------
vector<int> QTrackers::Update(vector<Point2d>& detections, Point2d frameSize)
{
	// -----------------------------------
	...
	if(tracks.size()==0)
	{
		...
		{
			QTrack* tr = new QTrack(detections[i],dt,Accel_noise_mag);
			tracks.push_back(tr);
		}
	}
	...
	vector<int> assignment; // assignment according to Hungarian algorithm
	...
	{
		double w=frameSize.x;
		double h=frameSize.y;
		double margin=0.485;
                //double margin=0.35;
		for(int i=0;i<not_assigned_detections.size();i++)
		{
			Point2f p=detections[not_assigned_detections[i]];
			if ((p.x>=margin*w && p.x<=(1-margin)*w) || (p.y>=margin*h && p.y <=(1-margin)*h)) continue;
                        //if ((p.y>=margin*h && p.y <=(1-margin)*h)) continue;
			QTrack* tr=new QTrack(detections[not_assigned_detections[i]],dt,Accel_noise_mag);
			tracks.push_back(tr);
		}
	}
	...
	for(int i=0;i<assignment.size();i++)
	{
		...
		if(assignment[i]!=-1) // If we have assigned detect, then update using its coordinates,ace
		{
