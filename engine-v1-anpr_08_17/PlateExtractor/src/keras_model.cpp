...
void keras::DataChunk2D::read_from_mat(const std::string &fname) {
	cv::Mat image = cv::imread(fname, cv::IMREAD_GRAYSCALE);
	cout << image.size() << endl;
	//ConvertMattoVectorFloat(img);
	//xuly
	/*if (image.channels() != 1)
	cvtColor(image, image, CV_BGR2GRAY);*/
	cv::resize(image, image, cv::Size(14, 28));
	std::vector<std::vector<float> > matrix1;
	for (int j = 0; j < image.cols; j++)
	{
		std::vector<float> rowmatrix;
		for (int i = 0; i < image.rows; i++)
		{
			rowmatrix.push_back((float)(image.at<uchar>(i, j)) / (float)255);
			cout << (float)(image.at<uchar>(i, j)) / (float)255 << " "; //white
		}
		cout << endl;
		matrix1.push_back(rowmatrix);
	}
	data.push_back(matrix1);
}
//
//void keras::DataChunk2D::mat2data(cv::Mat img) {
//	cv::Mat img_resize;
